...
- 主动消息分道只剩一处规则（`core/interruption.py`）：决策、提醒、你交代的
  结果、你保留的陪伴节奏和攒批清单实时进飞书；任何来源的「知道就行」（来信、
  行业动态、邮件周知、指标恢复、自己加的日程回显、自愈中的守护消息、工程
  迭代提案）只记账并在晨间锚点的一行汇总里点名。两周账本里这类卡约占实时
  投递的一半，且是「看不懂／没用」抱怨的全部来源。
- 日历变动只有两天内的取消/改期才是提醒卡，新增日程是周知；守护进程「我会
  继续处理」的消息不再是告警；指标异常声明为提醒卡、恢复为周知；好友欢迎
  失败不再把传输报错渲染进卡片；额度卡在用量过半前不写「预计用尽」；
  checkin 的裸 JSON 记账永不成卡。
- 飞书对话的系统提示带上版本化的 Codex/Jarvis 分工契约（`core/operating_model.py`
  v1.3.0），「什么时候需要你而不是 Codex」有了产品答案。
