...
- **缴回制度** (`core/memorial.py`, `tasks/memorial_escrow.py`)：从来没有任何东西
  ...
  死线全部由实测得出，不是拍的（7/01 起已批奏折）：decision 中位 2.1h、48h 内
  完成 90%、之后仅 3% → **48h**；alert 中位 0.2h、24h 内 78% → **24h**；
  notice 24/48/72h 响应率**持平在 48%** 而中位是 75h（当天点一批 + 隔几天手动
  扫一批）→ **7 天**，定短了会抢走他自己的扫尾行为。
  两种归宿刻意不同：**留中**是终态、静默、计数，且明确**不是决定**——台账绝不
  能读成他批过了；**匣子**把逾期 decision **按来源归并**成一张早间卡，绝不逐张
  重推（那是 7/22 的卡风暴）。归并才让积压可读：首个匣子 37 行其实只有 5 个
  来源，其中 14 行是同一个坏掉的流。
  有界：14 天没人批自动留中，匣子带「全部留中」一键归档。留中卡**仍可点**，
  点了会复活。Tier-0 设计——没有任何模型调用参与判断"他是否回应过"。
  首次生产实跑：留中 174、匣子 37、未发卡（当时在早间窗口外）。
