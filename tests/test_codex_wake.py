...
def test_wake_creates_verified_empty_task_without_acquiring_run(tmp_path):
    ...
    assert result["wake_receipt"]["id"] in start["developerInstructions"]
    assert "wake_id" in start["developerInstructions"]
...
def test_audit_detects_a_running_wake_without_its_authoritative_run():
    matter = create_matter("运行态不能悬空")
    link_entity(
        matter["id"],
        "session",
        "thread-running-without-run",
        provider="codex",
        metadata={
            "source": "codex_wake",
            "wake_id": "wake_without_run",
            "wake_status": "running",
            "workspace": "/tmp",
    ...
    report = audit_codex_wakes(now_epoch=500.0)

    assert report["healthy"] is False
    assert report["wake_states"]["running"] == 1
    assert report["issues"][0]["code"] == "running_wake_missing_run"
    assert report["issues"][0]["thread_id"] == "thread-running-without-run"
