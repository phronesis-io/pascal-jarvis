...
def test_plain_summary_writes_daily_log(tmp_path):
    (tmp_path / "hourly_log.md").write_text("### 10:00\nsome hourly entry\n")
    result = _run("- Morning check-in\n- Portfolio review", tmp_path)
    assert result.returncode == 0
    assert (tmp_path / "daily_log.md").exists()
    assert "Morning check-in" in (tmp_path / "daily_log.md").read_text()
    # Hourly should have been archived + cleared
    assert (tmp_path / "hourly_archive.md").exists()
    assert (tmp_path / "hourly_log.md").read_text() == ""
...
def test_summary_with_update_directives_does_not_crash(tmp_path):
    ...
    pending = tmp_path / "pending_updates.md"
    ...
    daily = (tmp_path / "daily_log.md").read_text()
...
def test_empty_input_noop(tmp_path):
    ...
    assert result.returncode == 0
    assert not (tmp_path / "daily_log.md").exists()
...
def test_updates_only_creates_pending_header(tmp_path):
    ...
    result = _run(stdin, tmp_path)
    assert result.returncode == 0
    pending = (tmp_path / "pending_updates.md").read_text()
