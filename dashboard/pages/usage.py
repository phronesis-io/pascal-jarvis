"""Usage page — local Claude Code usage panel.
...
def _tile(value: str, label: str, sub: str = "") -> None:
    with ui.card().classes("flex-1 p-3 min-w-[130px]"):
...
def _daily_bars_html(by_day: list[dict], n: int = 30) -> str:
    rows = by_day[-n:]
    if not rows:
        return "<div style='color:#9ca3af;font-size:13px'>暂无数据</div>"
    ...
    for r in rows:
        ...
        title = (
            f"{r['day']} · {r['turns']} turns · "
            f"out {fmt_tokens(r['out'])} · in {fmt_tokens(r['in'])} · "
            f"cache-r {fmt_tokens(r['cache_read'])}"
        )
        parts.append(
            "<div style='display:flex;align-items:center;gap:6px' "
            f"title='{html.escape(title)}'>"
            f"<span style='width:44px;font-size:11px;color:#6b7280;text-align:right'>{label}</span>"
            "<div style='flex:1;background:#f3f4f6;border-radius:4px;height:14px'>"
            f"<div style='width:{pct}%;background:#10b981;height:14px;border-radius:4px'></div>"
            "</div>"
            f"<span style='width:56px;font-size:11px;color:#374151'>{fmt_tokens(r['out'])}</span>"
            "</div>"
        )
    parts.append("</div>")
    return "".join(parts)
...
def _heatmap_html(heat: list[list[int]], heat_max: int) -> str:
    ...
    for h in range(24):
        ...
        header.append(
            f"<div style='width:14px;font-size:9px;color:#9ca3af;text-align:center'>{txt}</div>"
        )
    header.append("</div>")
    ...
    for w in range(7):
        row = [
            ...
            f"<span style='width:20px;font-size:10px;color:#6b7280'>{_WEEKDAYS[w]}</span>",
        ]
        for h in range(24):
            c = heat[w][h]
            if c:
                ...
                bg = "#f3f4f6"
            title = f"周{_WEEKDAYS[w]} {h:02d}:00 · {c} turns"
            row.append(
...
async def usage_page():
    with ui.column().classes("w-full max-w-5xl mx-auto p-4 gap-4"):
        with ui.row().classes("w-full items-center justify-between"):
            ...
            ui.label("Usage").classes("text-2xl font-bold")

        ui.label(
            "本机 Claude Code 用量：只读解析 ~/.claude/projects 的 transcript，"
            "只提数字（会话/token/活跃度），不读正文、不上传。含 workflow 子代理消耗。"
        ...
        def content():
            ...
            with ui.row().classes("w-full gap-3 flex-wrap"):
                ...
                _tile(f"{agg['files']:,}", "transcript 文件")
                span = ""
                if agg["first_ts"]:
            ...
            with ui.card().classes("w-full p-3"):
                ui.label("按模型").classes("text-sm font-semibold mb-2")
                mcols = [
                ...
                ui.table(columns=mcols, rows=mrows, row_key="model").classes("w-full")

            with ui.card().classes("w-full p-3"):
                ui.label("按项目").classes("text-sm font-semibold mb-2")
                pcols = [
                ...
                ui.table(columns=pcols, rows=prows, row_key="project").classes("w-full")
