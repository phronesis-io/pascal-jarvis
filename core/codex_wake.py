...
def _developer_instructions(matter_id: str, wake_id: str) -> str:
    return (
        ...
        "lease has started. Wait for the owner's message. When he asks to continue "
        "this outcome, call jarvis_matter_continue with this exact matter_id, the "
        f"exact wake_id {wake_id}, the current workspace, a concise task, and the "
        "actual desktop/mobile surface. Omit task_ref: Jarvis resolves the real "
        "Codex thread from this wake receipt. Follow the returned Context Packet "
        "and release or abort exactly once. Do not infer Matter completion."
    )
...
def audit_codex_wakes(
    *, now_epoch: float | None = None, stale_seconds: int = 300,
    ...
    linked: set[str] = set()
    wake_links: list[dict[str, Any]] = []
    link_rows = db.execute(
        "SELECT matter_id,entity_id,metadata FROM matter_links "
    ...
    for link in wake_links:
        metadata = link["metadata"]
        wake_status = str(metadata.get("wake_status") or "unknown")
        wake_states[wake_status] = wake_states.get(wake_status, 0) + 1
        lease_started = bool(metadata.get("matter_lease_started"))
        run_id = str(metadata.get("run_id") or "")
        if wake_status == "prepared" and lease_started:
            issues.append({
                "code": "prepared_wake_claims_lease",
                **{key: link[key] for key in (
                    "wake_id", "matter_id", "thread_id",
                )},
            })
            continue
        if wake_status != "running":
            if wake_status in {"released", "failed", "expired"} and lease_started:
                issues.append({
                    "code": "terminal_wake_claims_lease",
                    **{key: link[key] for key in (
                        "wake_id", "matter_id", "thread_id",
                    )},
                    "run_id": run_id,
        ...
        if not run_id:
            ...
            continue
        run = db.execute(
            "SELECT status,session_id FROM matter_runs WHERE id=?", (run_id,)
        ).fetchone()
        if run is None:
            issues.append({
                "code": "running_wake_missing_run",
        ...
        elif str(run["session_id"] or "") != link["thread_id"]:
            issues.append({
                "code": "wake_run_session_mismatch",
                **{key: link[key] for key in (
                    "wake_id", "matter_id", "thread_id",
                )},
                "run_id": run_id,
            })
        elif str(run["status"] or "") not in {"acquired", "running"}:
            issues.append({
                "code": "wake_projection_stale",
                **{key: link[key] for key in (
                    "wake_id", "matter_id", "thread_id",
                )},
                "run_id": run_id,
