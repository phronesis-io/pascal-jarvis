...
_LOCAL_ABSOLUTE_PATH_RE = re.compile(
    r"(?<![:/\w])(?:"
    r"/(?:Users|home|private|var|tmp|opt|Volumes)"
    r"(?:/[^\s`\"'<>|\[\]{}]+)+"
    r"|[A-Za-z]:\\(?:Users|Temp|Windows)"
    r"(?:\\[^\s`\"'<>|\[\]{}\\]+)+)",
    re.IGNORECASE,
)
...
def redact_text(value: object, limit: int = MAX_TURN_CHARS) -> str:
    ...
    text = _LOCAL_ABSOLUTE_PATH_RE.sub(_redact_local_path, text)
...
def _redact_local_path(match: re.Match) -> str:
    raw = match.group(0)
    clean = raw.rstrip(_PATH_TRAILING_PUNCTUATION)
    suffix = raw[len(clean):]
    name = clean.replace("\\", "/").rstrip("/").rsplit("/", 1)[-1]
    return f"[local-path]/{name or 'path'}{suffix}"
