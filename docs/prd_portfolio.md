...
| `prd_matter_workspace_mobile.md` | Superseded 2026-08-11 (REQ-120, REQ-123) | The Matter topic contract (cross-entry continuity, Matter detail) lives on in code. The mobile workspace surface is retired: the mobile gateway (:3458) and Tailscale funnel are gone, and Lark is the only delivery surface. Do not build against this document's phone/web surface sections. |
...
| `plans/2026-08-12-evidence-driven-attention-loop.md` | Current, implementation complete; rollout evidence in Taskline (REQ-124 to REQ-129) | Authoritative Lark-arrival accounting, a 25/day and 4/10-minute attention budget with durable overflow, explicit-only decisions, memorial-thread closure, plain user copy, and worktree-safe EigenFlux/memory harnesses. |
