...
| 135 | The honest Lark-to-Codex handoff still required manual task creation, while the historical Codex launcher acquired a six-hour Matter lease before the owner opened anything | The owner-private wake adapter now uses supported app-server methods to create, name, and read back a durable zero-turn task, commits its real thread ID as a Matter wake receipt, and leaves the run ledger untouched until the first owner message. The first continuation returns the exact generated wake ID, binds the real thread to the Matter Run, and advances the link through running and terminal states; stale projection is audited. Double actions reuse one unused task; partial creation deletes the orphan; uncertainty returns the stable phrase without claiming success. App-server, privacy, idempotency, cleanup, no-lease, exact-consumption, release/abort, and Lark natural-command regressions cover the path |
| 136 | The remaining manual Claude launcher handoff still acquired a six-hour run before execution, blocking immediate Codex continuation and `/done`; startup failures retained only the exception class | Every manual launcher handoff now writes a stable preview Context Packet and canonical command without creating a run; the command acquires and recompiles at actual execution. Repeated previews remain lease-free, a preview followed immediately by Codex continuation succeeds, and any post-acquire startup failure is aborted with a bounded concrete reason in its receipt and structured log |
| 137 | Completion capture could raise after the provider exited and leave the Matter Run active; one transient renewal failure permanently stopped the keeper, the keeper shared the process singleton transaction, and weekly review counted expired leases as live work | The launcher now aborts on completion-receipt failure, renewal uses an independent WAL connection with bounded retry, and weekly review recovers stale ownership before reading while every active-run predicate checks the actual expiry. Completion-failure, transient-retry, 1,000-operation renew/abort race, expired-review, and pre-hook ordering regressions cover the failure class |
| 138 | Two early quota samples produced a linear exhaustion forecast that promoted 7–23% real usage to `critical`, while a transient package-read failure cleared and re-armed the same alert; reset rollover also minted duplicate episodes | Forecast eligibility now requires at least 50% usage or 25% elapsed window and remains informational; only exact usage of 90% or an authoritative limit flag creates proactive attention. Unknown reads preserve state and reset timestamps are excluded from episode identity. The production 10→14→18→23% observation sequence, low/mature forecasts, read failure, reset rollover, recovery, and exhausted transition all have deterministic regressions |
| 139 | Governed deploy could install Python packages and mutate `~/.codex`, and any Codex CLI/login/output/handshake failure aborted the resident Jarvis release before restart | Setup now owns MCP dependencies and plugin registration. Deploy performs only JSON-based installed/enabled readback plus a read-only stdio smoke, wraps all frontstage degradation as a non-blocking warning, and continues the governed resident restart. Shell lifecycle tests pin mutation ownership, setup ordering, JSON verification, and the non-blocking deploy branch |
| 140 | EigenFlux private mail put the peer quotation before Jarvis's judgment, repeated a branding footer, and sent five separate cards inside 32 minutes; later mail could not join an already queued or delivered card without losing its own receipt | PM cards now lead with one bounded judgment, carry one quoted source line, omit the footer, retain every canonical message ID, and share one hourly memorial identity. A ledger `revise` event updates the same pending Item, queued delivery payload, and already-delivered Lark card through the keychain-independent bot PATCH API. Exact dedup, recovery lookup, one-envelope aggregation, terminal non-reopen, queued replacement, direct PATCH, and historical three-message polling regressions cover the path |
| 141 | Protected CI exercised Python 3.12 while every resident process ran 3.14, UTC runners skipped the two timezone regressions, and the local one-command gate bypassed the coverage ratchet entirely | The branch-protected `test` job keeps its exact check identity while moving to Python 3.14; a separate full Python 3.12 compatibility job preserves the lower runtime promise. Both jobs set the host timezone to Asia/Shanghai before tests, and the default local gate now records branch coverage and runs the same reviewed budget as CI. Contract tests pin the check name, both interpreters, real host timezone setup, and local coverage commands |
| 142 | A transient GitHub outage blocked every governed restart, while stale operational memory still told agents to bypass `restart.sh` and invoke launchd directly without joined release/runtime evidence | A successful live gate now atomically stores a private exact-SHA receipt for at most 24 hours. Only recognized network unavailability may reuse it, with `stale` and age preserved in the final release receipt; changed SHA, stale/unsafe cache, auth, policy, CI, or review failures remain closed. Operational memory now names `restart.sh --full` as the sole code-release entry and bans raw kickstart; source-order and adversarial cache tests pin both sides |
