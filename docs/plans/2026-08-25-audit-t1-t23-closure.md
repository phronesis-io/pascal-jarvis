...
| T2 | Keep the reusable prompt prefix cacheable | Stable memory precedes volatile state; indexed warm memory is the default; cache usage ratios are queryable; exact per-session/per-trust-profile system snapshots now respect the provider's content-block cache boundary. The 2026-08-26 live metric disproved the earlier “dynamic suffix is safe” assumption and reopened this row. | **code-complete, runtime-pending**: after the corrective release, the second live compatible call must show the expected cache read/write ratio. |
| T3 | Prevent timeout retry storms and fake health probes | One logical call has one wall-clock budget; production calls never act as probes; safe no-tool work advances through Primary, Backup1, Backup2, then GPT within the remainder; every relay has a bounded attempt cap; ambiguous tool timeouts fail closed; provider failures do not poison non-heavy task circuits; recovery probes remain in `provider-canary`. Explicit Sonnet/Haiku task tiers stay at that tier across relays. | **code-complete, runtime-pending**: the 2026-08-26 production incident reopened and corrected the budget contract; run real read-only canaries and verify no new full-budget relay stall after release. |
...
| T6 | Detect detached self-improvement sessions that produced nothing | The session now persists acquire/run/release receipts, reconciles missing release, retries on a bounded clock, records usage, and reports health only after repeated failures. A worker must pass a final run-id/status/lease admission check before model execution; terminal receipts are write-once; an expired live worker is fenced by command identity before TERM/KILL with exit confirmation. The harness also inherits a private process-lease descriptor, so controller recovery can find an ordinary double-forked child after it closes stdio and leaves the candidate cwd. A reused PID is never signalled, and an unavailable identity probe retains the exclusive lease instead of guessing. | **code-complete**. |
...
| T10 | Stop rebuilding roughly 190k tokens on every Lark message | Resumed conversations reuse an exact mode-0600 system snapshot keyed by session, trust/context boundary, memory budget, and prompt implementation. Current time stays only in the message body; recent transcript, compact output, and current-message focus are not rebuilt into a resumed system block. Queue/rotation and backup-provider rebuilds select the correct snapshot variant. | **code-complete, runtime-pending**: verify two live resumed messages reuse the system block. |
...
| T20 | Stop memory maintenance damaging or lying about memory | Weekly digest preserves the rolling daily log; tidy measures production index and reference full modes separately; exact allowlisted stale operational claims are migrated; due intents retrieve bounded verbatim warm evidence. Cross-session findings flatten nested model-authored card directives before the strict work-receipt boundary, so a second `TITLE` can no longer leave a title-only empty card. | **code-complete**: stale runtime claims migrate on the first post-release tidy. |
...
| T22 | Stop Guardian restart loops and invisible recovery state | No self-mtime reload; failed `ps` is unknown; listener recovery targets only its sidecar; startup/wake grace covers critical probes; recurring incident dedup is 24h; only Jarvis-owned live session locks (or their short acquire window) block whole-stack restart, so stale lock files cannot hide a dead heartbeat; bot receives graceful SIGTERM before bounded cleanup; repeated DEGRADED logs emit only on state change; external dead-man withholds green when brain-dead. EigenFlux health now accepts either verified real-time ingress or its five-minute poll, Guardian leaves a live self-reconnecting stream alone during network/VPN degradation, and runtime-version drift cannot be reported as a successful child-recovery request. | **code-complete, runtime-pending**. New recovery chatter/card types were rejected under the current low-noise policy; receipts and private logs retain attribution instead. |
| T23 | Bound operational state and protect private files | Daemon logs retain 30 generations; log maintenance runs allowlisted mode repair, 90/180-day SQLite detail retention, temp cleanup, and memory Git auto-gc; no raw payload is added to state; no active-writer VACUUM is attempted; external dead-man checks brain and delivery transport. Primary Anthropic, relay, and OpenAI credentials are no longer globally exported by `bot.sh`: the heartbeat routing worker receives the configured route set, each direct Claude/OpenAI adapter receives only its active credential, ordinary pre/post scripts and Codex/GPT-started tools get a scrubbed environment. | **code-complete, config-dependent** for live provider canaries. |
