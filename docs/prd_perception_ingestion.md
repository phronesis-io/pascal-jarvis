...
- **跨session记忆引用说明**:`project_self_evolving_repo` / `project_eigenflux_native_parity` / `feedback_verify_write_paths` / `feedback_llm_output_boundary` / `feedback_autonomous_memory_absorption` / `feedback_memory_design_truememory` 等位于 `~/.claude/projects/-Users-pascal-Desktop-jarvis-repos-pascal-jarvis/memory/`(**非 repo 内文件**,repo-only 读者需到该目录查;相关规则已在正文内联)。
