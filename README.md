...
11. **Items, Topics & Mobile Home** — `/items` is the canonical visual inbox: a Memorial is the only user-facing Item, Matter is an optional topic filter and handoff context, and Intent appears only as a timed-reminder attribute. Ordinary decisions wait for phone/web batch review while Lark stays a sparse conversation and immediate-decision channel. A durable Matter still connects Lark, Claude/Codex sessions, jobs, and artifacts under the surface. All Memorial, heartbeat, bot-reply, and Guardian output now crosses one SQLite-backed delivery state machine with global sanitization, 6-hour deduplication, throttling, quiet hours, retry, dead-letter, and delivered/read/acted confirmation. An authenticated TLS gateway on `:3458` gives paired phones a revocable, audited PWA; pairing automatically creates a test notice. Tailscale Funnel exposes only that gateway, never `:3456` or `:3457`. See [the unified delivery PRD](docs/prd_unified_delivery_items.md) and [the Matter/mobile PRD](docs/prd_matter_workspace_mobile.md).
