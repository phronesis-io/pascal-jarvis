...
11. **Matter Workspace & Mobile Home** — A durable Matter connects Lark conversations, Claude/Codex sessions, memorials, intents, background jobs, and artifacts without copying their native stores. `/matters` is the visual home; `/matter ...` and `/model` provide deterministic Lark controls; `scripts/jarvis-matter` hands bounded context to either executor. An authenticated TLS gateway on `:3458` gives paired phones a revocable, audited PWA and Web Push path while keeping `:3456` and `:3457` on loopback. See [the Matter workspace PRD](docs/prd_matter_workspace_mobile.md).
