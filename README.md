...
6. **Attention Engineering** — Proactive output is classified before delivery: explicit choices become decision-first memorial cards (奏折), urgent non-choice alerts may still reach Lark, and routine FYI output stays in the web `知会` stream. Every record remains in the durable ledger and can enter conversation through 「聊聊这个」. Quiet hours, batching, deduplication, and aggregated delivery-failure alerts protect the same attention budget without turning every update into a pending decision.
...
- Configure `claude.backup_auth_token` + `claude.backup_base_url` to retry `claude -p` through a Claude Code-compatible provider after the primary Claude chain fails.
- Configure `openai.api_key` or `OPENAI_API_KEY` for the final GPT fallback. This OpenAI fallback cannot run Claude Code tools, inspect local files, or edit code; it is meant to keep ordinary conversation alive during Claude outages.
