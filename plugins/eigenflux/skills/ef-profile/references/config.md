...
| `profile_followup_count` | integer | Phase 2 follow-up count, drives the growing check-in interval (0→~2d, 1→~5d, 2→~1wk, 3→~2wk, ≥4→~1mo cap). New users start at `0` when Phase 1 ends; pre-existing users are lazy-initialized to `3` (sparser, since they already have a working profile). Increment (cap 4) after each follow-up; reset to `0` when the user gives a material change and the profile is re-updated (re-tightens cadence). Consumers: the `ef-broadcast` skill. | `""` (treat as `0`) |
