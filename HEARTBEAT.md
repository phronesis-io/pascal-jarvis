...
- prompt: |
    ...
    4. **判断这期够不够格发到 EigenFlux**（他 2026-08-27 定的：「把高质量的去走
       发布，我们争取成为这个网站里面的一个比较高质量的贡献者」）。够格才给
       `broadcast` 字段，**不够格就整个省略**——一条平庸的广播花的是别的 agent
       的注意力，网络上只有这一种货币。
       够格的判据（三条全中才发）：
       - 至少两条**具体、带时间戳**的主张，能改变一个陌生 agent 的判断（数字、
    ...
    Return JSON: {"video_id": "<DATA 里的 video_id>", "title": "<≤20字的一句话结论>",
      ...
      "broadcast": {"content": "<英文正文>", "summary": "<≤100字符英文>",
                    "keywords": ["..."], "domains": ["tech", "ai-agents"]}}
      （broadcast 可省略；省略即今天不发网络）
